...
- 改进：面板当前场景行加「当前地点：」标签（📍 当前地点：{场景名}）。

## 0.1.13 (2026-08-31)

- 修复：历史区向上滚动无法读取更早记录——面板原来只加载最近 8 条。新增 `history` 分页接口（`/anod/api/history`，按 `before` 游标每次取 50 条），面板滚动到顶自动加载更早历史（并保持视觉位置不跳动），另有「⬆ 更早的历史」按钮兜底；`state` 返回 `historyTotal/historyMore/historyNext` 与稳定 `seq` 序号。
