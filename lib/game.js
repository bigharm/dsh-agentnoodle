...
// deps: { dataDir, readText(path)→string|null, writeText(path, content), listDirNames(dir)→string[], readBinary(path)→Uint8Array|null, copyFile(from,to), callAI(prompt, cfg)→string, log(msg) }
export function createGame(deps) {
  const { dataDir, readText, writeText, listDirNames, readBinary, copyFile, callAI, log } = deps
  ...
  const FALLBACK_PROMPT = [
    ...
    '- is_dead: 布尔值',
    '- new_location: 玩家位置变化时为新场景名，否则 null。若玩家明确表示前往某处（如"前往地牢"），即使该场景不在已有地点库中，也应让玩家成功抵达并返回该新场景名（系统会自动注册进地点库）',
    '- relationship_update: 态度显著变化时返回 {"NPC名":"新态度"}，否则 null',
    '只输出 JSON。',
  ].join('\n')

  function readRules(worldId) {
    const text = readText(dataDir + '/worlds/' + worldId + '/rules.txt')
  ...
  function buildActPrompt(char, world, sceneName, userInput, cfg) {
    ...
    return replaceAll(template, {
      game_rules: readRules(char.world_id || '') || '（无）',
  ...
  // 面板展示：state 只带最近 N 条 + 翻页游标；更早历史走 history 分页接口
  const STATE_TAIL = 20
  ...
  function handleState(args) {
    ...
    const loc = locationByName(world.locations, sceneName)
    const npcs = npcsAtScene(world.npcs, loc)
    const full = char.conversation_history || []
    const tail = full.slice(-STATE_TAIL)
    const base = full.length - tail.length
    return {
      character: {
      ...
      historyTotal: full.length,
      historyMore: base > 0,
      historyNext: base,
    }
  }

  // 更早历史分页：before 为起点下标（不含，即上次 chunk 的起始位置），返回其之前的 limit 条
  function handleHistory(args) {
    ...
    const h = char.conversation_history || []
    const limit = Math.min(Math.max(Number(args.limit) || HISTORY_PAGE, 1), 200)
    let end = h.length
    const before = Number(args.before)
    if (Number.isFinite(before) && before > 0 && before < end) end = Math.floor(before)
    const start = Math.max(end - limit, 0)
    const entries = h.slice(start, end).map((m, k) => ({
      seq: start + k,
      speaker: m.speaker || '',
      content: m.content || '',
      avatar: m.avatar || '',
    ...
    return { entries, more: start > 0, nextBefore: start, total: h.length }
  }
  ...
  function handleUpdateRules(args) {
    ...
    try { loadWorld(worldId) } catch (e) { return { error: '加载世界失败: ' + String(e) } }
    const rulesText = String(args.rulesText === undefined ? '' : args.rulesText)
    // 规范化：去掉行尾空白与首尾空行，保留 # 注释行
    const cleaned = rulesText.split(/\r?\n/).map((l) => l.replace(/\s+$/, '')).join('\n').replace(/^\n+|\n+$/g, '')
    writeText(dataDir + '/worlds/' + worldId + '/rules.txt', cleaned)
    const count = cleaned.split('\n').filter((l) => l.trim() && !l.trim().startsWith('#')).length
    return { ok: true, worldId, ruleCount: count, note: '规则已更新，下次行动立即生效' }
  }
  ...
  async function generateLocations(worldId, world, cfg) {
    ...
    const prompt = replaceAll(template, { world_setting: world.worldview, game_rules: readRules(worldId) || '（无）' })
    const raw = await callAI(prompt, cfg)
    const parsed = cleanJson(raw)
  ...
  async function generateNpcs(worldId, world, cfg) {
    ...
    const prompt = replaceAll(template, { world_setting: world.worldview, locations: locList, game_rules: readRules(worldId) || '（无）' })
    const raw = await callAI(prompt, cfg)
    const parsed = cleanJson(raw)
  ...
  // ---------- 角色卡/世界导入（兼容 SillyTavern V1/V2/V3 JSON 与 PNG 内嵌卡片） ----------
  function readU32BE(bytes, offset) {
    return (bytes[offset] << 24 | bytes[offset + 1] << 16 | bytes[offset + 2] << 8 | bytes[offset + 3]) >>> 0
  }

  function parseTavernCardPng(bytes) {
    ...
    while (offset + 12 <= bytes.length) {
      ...
      if (type === 'tEXt' && dataStart + len <= bytes.length) {
        let nul = dataStart
        while (nul < dataStart + len && bytes[nul] !== 0) nul++
        const keyword = Buffer.from(bytes.slice(dataStart, nul)).toString('ascii')
        if (keyword === 'chara' || keyword === 'ccv3') {
          const value = Buffer.from(bytes.slice(nul + 1, dataStart + len)).toString('utf8')
          try {
            const decoded = Buffer.from(value, 'base64').toString('utf8')
            return normalizeTavernCard(JSON.parse(decoded))
          } catch (e) {
            try { return normalizeTavernCard(JSON.parse(value)) } catch (e2) { return null }
          }
        }
      }
  ...
  function normalizeTavernCard(json) {
    ...
    return {
      ...
      firstMes: String(data.first_mes || '').trim(),
    }
  }

  function handleImportCard(args) {
    const worldId = safeId(args.worldId || '')
    const nameOverride = String(args.name || '').trim()
    if (!worldId) return { error: 'worldId 必填' }
    let world
    try { world = loadWorld(worldId) } catch (e) { return { error: '加载世界失败: ' + String(e) } }
    // 解析卡片来源：filePath（PNG/JSON 文件）或 cardText（粘贴的 JSON）
    let card = null
    if (args.filePath) {
      const bytes = readBinary(String(args.filePath))
      if (!bytes) return { error: '读取文件失败: ' + args.filePath }
      if (bytes[0] === 0x89 && bytes[1] === 0x50) {
        card = parseTavernCardPng(bytes)
        if (!card) return { error: 'PNG 中未找到 SillyTavern 卡片数据（tEXt chara 块）' }
      } else {
        try { card = normalizeTavernCard(JSON.parse(Buffer.from(bytes).toString('utf8'))) } catch (e) { return { error: '文件不是有效的卡片 JSON' } }
      }
    } else {
      const text = String(args.cardText || '').trim()
      if (!text) return { error: '需要 cardText（粘贴卡片 JSON）或 filePath（本地 PNG/JSON 卡路径）' }
      try { card = normalizeTavernCard(JSON.parse(text)) } catch (e) { return { error: '卡片 JSON 解析失败: ' + String(e) } }
    }
    if (!card || !card.name) return { error: '卡片缺少 name 字段' }
    const loc = args.locationId ? locationByName(world.locations, String(args.locationId)) : (world.locations.locations || [])[0]
    if (!loc) return { error: '需要有效的 locationId（世界暂无地点，先用 generate_content 或 add_location 生成）' }
    const traits = card.personality ? card.personality.split(/[,，、\n]/).map((s) => s.trim()).filter(Boolean).slice(0, 6) : []
    const npc = {
      id: 'npc_' + Date.now(),
      name: nameOverride || card.name,
      gender: '',
      profile: {
        identity: card.description ? card.description.split('\n')[0].slice(0, 60) : '神秘来客',
        description: card.description || '',
        personality_traits: traits,
        background: [card.scenario, card.personality].filter(Boolean).join('\n') || '',
      },
      location_id: loc.id,
      active: true,
      dead: false,
    }
    if (card.firstMes) npc.first_mes = card.firstMes
    world.npcs.npcs = world.npcs.npcs || []
    world.npcs.npcs.push(npc)
    writeJson(dataDir + '/worlds/' + worldId + '/npcs.json', world.npcs)
    return { ok: true, id: npc.id, name: npc.name, locationId: loc.id, locationName: loc.name, hasFirstMes: !!npc.first_mes, note: '角色卡已导入为 NPC；first_mes 已存档备用' }
  }

  function handleImportWorld(args) {
    const sourceDir = String(args.sourceDir || '').trim()
    const worldId = safeId(args.worldId || '')
    if (!sourceDir || !worldId) return { error: 'sourceDir 和 worldId 必填' }
    const worldview = readText(sourceDir + '/worldview.txt')
    if (worldview === null) return { error: '源目录缺少 worldview.txt（不是有效的 LazyNoodle 世界目录）' }
    const locations = readJson(sourceDir + '/locations.json') || { regions: [], locations: [] }
    const npcs = readJson(sourceDir + '/npcs.json') || { npcs: [] }
    const idx = loadWorldsIndex()
    if ((idx.worlds || []).some((w) => w.id === worldId)) return { error: '世界已存在: ' + worldId }
    writeText(dataDir + '/worlds/' + worldId + '/worldview.txt', worldview)
    writeJson(dataDir + '/worlds/' + worldId + '/locations.json', locations)
    writeJson(dataDir + '/worlds/' + worldId + '/npcs.json', npcs)
    // 复制头像目录（若有）
    let avatarCount = 0
    if (copyFile) {
      const avatarFiles = listDirNames(sourceDir + '/avatars').filter((f) => /\.(png|jpe?g|webp)$/i.test(f))
      for (const f of avatarFiles) {
        try {
          copyFile(sourceDir + '/avatars/' + f, dataDir + '/worlds/' + worldId + '/avatars/' + f)
          avatarCount++
        } catch (e) { /* 跳过单个失败头像 */ }
      }
    }
    idx.worlds = idx.worlds || []
    idx.worlds.push({ id: worldId, name: String(args.name || worldId), description: String(args.description || ''), created_at: new Date().toISOString(), last_played: null })
    idx.current_world = worldId
    writeJson(dataDir + '/worlds/worlds_index.json', idx)
    return {
      ok: true, worldId, name: String(args.name || worldId),
      locations: (locations.locations || []).length, npcs: (npcs.npcs || []).length, avatars: avatarCount,
      note: '世界已导入并切换为当前世界',
    }
  }
  ...
  return {
    ...
    importCard: handleImportCard,
    importWorld: handleImportWorld,
  }
}
